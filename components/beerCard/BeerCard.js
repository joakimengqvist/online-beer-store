      ...
      <Card.Header className={styles.beerCardHeader}>
        <Nav>
          <Nav.Item
            className={joinClassNames(
              styles.beerTab,
              showingTab === "main" && styles.activeBeerTab
            )}
          >
            <Nav.Link
              onClick={() => {
                !isBanned && setShowingTab("main");
              }}
            >
              {isBanned ? (
          ...
          <Nav.Item
            className={joinClassNames(
              styles.beerTab,
              showingTab === "ingredients" && styles.activeBeerTab
            )}
          >
            <Nav.Link
              onClick={() => {
                !isBanned && setShowingTab("ingredients");
              }}
            >
              Ingredients
            </Nav.Link>
                ...
                <Card.Text>
                  ...
                  {Array.isArray(beer.ingredients[ingredientType])
                    ? beer.ingredients[ingredientType].map((ingredient) => (
                        <span
                          className={styles.beerParagraph}
                          key={ingredient.id}
                        >
                          {ingredient.name}{" "}
                        </span>
              ...
              <img
                ...
                className={
                  beer.image_url === "https://images.punkapi.com/v2/keg.png"
                    ? styles.kegBeerCardImage
                    : styles.beerCardImage
                }
              />
            )}
          </Col>
