        ...
        case STATUSES.index: {
            ...
            if (event.type === EVENTS.ADD_ITEM_TO_CART) {
                ...
                if (cart[beerIdInCart]) {
                    ...
                    return {
                        ...state,
                        cart: cart,
                        effects: [...state.effects, createEffect(EFFECTS.triggerToast, {action: TOAST_TYPES.add, beerName: beerName})],
                    }   
                }
            ...
            if (event.type === EVENTS.REMOVE_ITEM_FROM_CART) {
                ...
                return {
                    ...
                    cart: cart,
                    itemsInCart: state.itemsInCart - 1,
                    effects: [...state.effects, createEffect(EFFECTS.triggerToast, {action: TOAST_TYPES.remove, beerName: beerName})],
                }   
            }
            ...
            if (event.type === EVENTS.CHANGE_ITEM_QUANTITY) {
                ...
                if (quantityString === '') {
                    return {
                        ...state,
                        effects: [...state.effects, createEffect(EFFECTS.removeItemFromCart, {itemId: event.itemId})],
                    }
                }
                ...
                if (!/^[0-9]+$/.test(quantityString)) {
                    return {
                        ...state,
                        effects: [...state.effects, createEffect(EFFECTS.triggerToast, {action: TOAST_TYPES.invalidQuantity})],
                    };
                }
                ...
                return {
                    ...state,
                    cart: cart,
                    effects: [...state.effects, createEffect(EFFECTS.triggerToast, {action: TOAST_TYPES.change, beerName: cart[event.itemId].name, quantity: quantity})],
                }   
            }
            ...
            if (event.type === EVENTS.CLEAR_CART) {
                ...
                return {
                    ...
                    cart: {},
                    itemsInCart: 0,
                    effects: [...state.effects, createEffect(EFFECTS.triggerToast, {action: TOAST_TYPES.clear})],
                }
            }
        ...
        for (const effect of effects) {
            ...
            if (effect.type  === EFFECTS.triggerToast) {
                ...
                if (effect.action  === TOAST_TYPES.add) {
                    toast.success(`Added ${effect.beerName} to cart`, toastSettings("bottom-right")); 
                }

                if (effect.action  === TOAST_TYPES.remove) {
                    toast.info(`Removed ${effect.beerName} to cart`, toastSettings("bottom-right"));
                }

                if (effect.action  === TOAST_TYPES.change) {
                    toast.success(`Changed ${effect.beerName} to ${effect.quantity}`, toastSettings("bottom-right"));
                }
